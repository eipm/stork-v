...
zip_file = '56577073.zip'
embryo_num = zip_file[0:len(zip_file)-4]
focus = 0

input_directory = zip_file
target_directory = '/<directory to unzip images to>/' + embryo_num
with zipfile.ZipFile(input_directory,"r") as zip_ref:
    zip_ref.extractall(target_directory)
image_folder = '<directory to unzip images to>/' + embryo_num + "/cleaned_good" + "/" + embryo_num
...
if len(filtered_ims) > 10:
    video_name = "/<directory to store video>/" + embryo_num + "_" + str(focus) + '.avi'
...
root_dir = "/<directory where video is stored>/"
...
model_bs_filepath = "<filepath to BiLSTM model>"
model_lr_filepath_eup_anu = "<filepath to logistic regression model for EUP vs ANU>"
model_lr_filepath_eup_cxa = "<filepath to logistic regression model for EUP vs CxA>"
